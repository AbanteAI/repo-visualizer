  ...
  const renderComponent = (component: any, depth = 0) => {
    return (
      <div key={component.id} className="ml-4">
        <div className="font-medium">{component.name} <span className="text-gray-500 text-sm">({component.type})</span></div>
        {component.lineStart && component.lineEnd && (
          <div className="text-sm text-gray-600">Lines {component.lineStart}-{component.lineEnd}</div>
        )}
      ...
      <div className="border-b border-gray-200 pb-2 mb-2">
        ...
        {file.type === 'file' && (
          <>
            ...
            {file.extension && <p><span className="font-medium">Extension:</span> .{file.extension}</p>}
          </>
        )}
        
        <p><span className="font-medium">Depth:</span> {file.depth}</p>
