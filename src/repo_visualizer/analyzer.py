...
class RepositoryAnalyzer:
    ...
    def _get_file_coverage(self, file_path: str) -> Optional[float]:
        ...
        """
        try:
            # Try to find Python coverage data
            if file_path.endswith(".py"):
                return self._get_python_coverage(file_path)

            # Try to find JavaScript/TypeScript coverage data
            elif file_path.endswith((".js", ".ts", ".jsx", ".tsx")):
                return self._get_js_coverage(file_path)

        except Exception as e:
            print(f"Error getting coverage for {file_path}: {e}")

        return None

    def _get_python_coverage(self, file_path: str) -> Optional[float]:
        """Get Python coverage from coverage.py data files."""
        ...
        if os.path.exists(coverage_file_path):
            try:
                ...
                if analysis:
                    ...
                    missing = analysis[3]  # missing statements
                    if statements:
    ...
    def _get_js_coverage(self, file_path: str) -> Optional[float]:
        """Get JavaScript/TypeScript coverage from various sources."""
        ...
        if os.path.exists(lcov_path):
            try:
                ...
                for section in sections:
                    if f"SF:{file_path}" in section or file_path in section:
                        lines = section.split("\n")
                        found_lines = 0
                        hit_lines = 0
        ...
        if os.path.exists(coverage_summary_path):
            try:
                ...
                # Find file in coverage summary
                for file_key, file_data in data.items():
                    if isinstance(file_data, dict) and (
                        file_key.endswith(file_path) or file_path.endswith(file_key)
                    ):
                        lines_data = file_data.get("lines", {})
                        pct = lines_data.get("pct", 0)
                        return pct / 100.0  # Convert percentage to ratio
